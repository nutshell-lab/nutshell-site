<template>
  <v-layout row wrap justify-center>
    <v-flex xs11 sm10 lg8 class="py-3 my-5 pt-4">
      <v-layout row wrap align-center>
        <v-carousel height="230" hide-controls class="elevation-0 references">
          <v-carousel-item v-for="(ref, index) in refs" :key="index">
            <v-layout row wrap align-content-center class="reference">
              <v-flex xs12 class="text-xs-center primary--text headline">
                <span class="secondary--text">"</span> {{ ref.comment }} <span class="secondary--text">"</span>
              </v-flex>
              <v-flex xs12 class="text-xs-center primary--text headline mt-3">
